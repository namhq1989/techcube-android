...
public class Transformations {
     ...
     * </pre>
     *
     * @param result a LiveData which emits resulting values
     * @param source a {@code LiveData} to listen to
     * @param func   a function to apply
     * @param <X>    a type of {@code source} LiveData
     * @param <Y>    a type of resulting LiveData.
     */
    @MainThread
    public static <X, Y> void map(MediatorLiveData<Y> result, LiveData<X> source, final Function<X, Y> func) {
        result.addSource(source, new Observer<X>() {
            @Override
            public void onChanged(@Nullable X x) {
                result.setValue(func.apply(x));
     ...
     */
    @MainThread
    public static <X, Y> void switchMap(MediatorLiveData<Y> result, LiveData<X> trigger,
                                        final Function<X, LiveData<Y>> func) {
        result.addSource(trigger, new Observer<X>() {
            LiveData<Y> mSource;
